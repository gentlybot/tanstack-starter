...
// pg-boss stores its queues in Postgres (in a `pgboss` schema), so background
// jobs need no extra infrastructure beyond the database the app already has.
...
/** Enqueue a job from anywhere on the server (server functions, API routes). */
